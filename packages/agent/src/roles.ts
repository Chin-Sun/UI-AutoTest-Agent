...
export const ROLES: Record<RoleName, RoleDef> = {
  ...
  compiler: {
    name: 'compiler',
    persona: '你是用例编译 Agent：把自然语言测试用例编译成 Playwright 可执行的 Step DSL。你不执行测试，只输出计划。',
    tools: ['list_components', 'load_skill', 'propose_plan'],
  ...
  repairer: {
    name: 'repairer',
    persona: '你是用例修正 Agent：根据人的反馈修正测试计划。只改与反馈相关的步骤，其余步骤与 id 保持不变。',
    tools: ['load_skill', 'propose_plan'],
    terminalTool: 'propose_plan',
    requiredSkills: ['case-to-steps'],
  },
  reporter: {
