...
export function GatePage({ projectId, findings, go }: { projectId: string; findings: Finding[]; go: Go }) {
  ...
  return (
    <div className="page">
      <header><h1>失败门禁</h1><p>步骤错误与缺少数据会停在这里等人补充。提交后 Agent 修正用例计划（或写入数据）并只重跑该用例；小修自动批准，改结构或预期需要回到②批准。</p></header>
      ...
      {recent.length > 0 && <section className="card">
        <h2>最近关闭</h2>
        <table><tbody>{recent.map((f) => <tr key={f.id}><td>{cases.find((c) => c.id === f.caseId)?.title}</td><td><Pill tone={f.verdict}>{VERDICT_LABEL[f.verdict]}</Pill></td><td>{FINDING_STATUS_LABEL[f.status]}</td><td className="small muted">R{f.round} · {f.summary}</td></tr>)}</tbody></table>
      </section>}
    </div>
  )
}
...
function GateCard({ finding, testCase, go }: { finding: Finding; testCase?: TestCase; go: Go }) {
  ...
  useEffect(() => { void api<StepPlan>(`/api/plans/${finding.planId}`).then(setPlan) }, [finding.planId])
  ...
  const upload = async (key: string, file: File) => {
    ...
    const { path } = await api<{ path: string }>('/api/uploads', { body: { filename: file.name, base64 } })
  ...
  return (
    <section className={`card finding ${finding.status}`}>
      ...
      <p className="small">{finding.evidence.video && <a href={fileUrl(finding.evidence.video)} target="_blank">录像</a>} {finding.evidence.trace && <> · <a href={fileUrl(finding.evidence.trace)}>trace</a></>}</p>

      {busy ? <div className="actions">
        <span className="muted">{finding.status === 'repairing' ? 'Agent 正在修正计划；若修正涉及结构/预期变更，需要到②批准' : '已排队重跑'}</span>
        <button className="ghost" onClick={() => go(finding.status === 'repairing' ? 'plans' : 'run', finding.status === 'repairing' ? finding.caseId : undefined)}>查看 →</button>
      </div> : finding.verdict === 'data-missing' ? <div className="form">
        {(finding.missingKeys ?? []).map((key) => (
          <label key={key}>{key}
            <div className="row">
              <input value={data[key] ?? ''} onChange={(e) => setData({ ...data, [key]: e.target.value })} placeholder="值或文件路径" />
              <input type="file" onChange={(e) => { const f = e.target.files?.[0]; if (f) void upload(key, f) }} />
            </div>
          </label>
        ))}
        <div className="actions">
        ...
        <label>指点 Agent（用「」标出正确的页面文字，或直接改上面的步骤）
          <textarea rows={2} value={hint} onChange={(e) => setHint(e.target.value)} placeholder="例如：按钮叫「保存」，不是「提交」" />
        </label>
        <div className="actions">
          <AsyncButton disabled={hint.trim() === '' && patch === undefined} onClick={() => send({
            kind: 'fix-step', content: hint,
      ...
      <LiveLog scopes={[`repair:${finding.id}`, `run:${finding.runId}`]} title="Agent / 执行日志" />
