...
import { AsyncButton, Pill, TokenUsage } from '../components'
...
export function ReviewPage({ projectId, findings }: { projectId: string, findings: Finding[], go: Go }) {
  ...
  const loadReports = useCallback(() => {
    void api<Report[]>(`/api/reports?projectId=${projectId}`).then(setReports)
  }, [projectId])
  useEffect(() => {
  ...
  return (
    <div className="page">
      <header>
        <h1>结果审阅与报告</h1>
        <p>动作全部执行成功、但结果与预期不符的用例进入这里。确认后计入报告；判定“不是缺陷”时写明正确预期，Agent 会据此修正用例。</p>
      </header>
      ...
      <section className="card">
        <div className="case-head">
          ...
          <div className="actions">
            <TokenUsage scopes={[`report:${projectId}`]} />
                ...
                <tbody>
                  {reports.map((r) => (
                    <tr key={r.id} title={r.blockers.join('\n')}>
                      ...
                      <td>{r.summary.cases}</td>
                      <td>{r.summary.passed}</td>
                      <td>{r.summary.failed}</td>
                      <td>{r.summary.defects}</td>
                      <td>{r.summary.open}</td>
                      <td><a href={fileUrl(r.html)} target="_blank" rel="noreferrer">打开 →</a></td>
                    </tr>
                  ))}
                </tbody>
              </table>
  ...
  return (
    <section className="card finding awaiting_review">
      <div className="case-head">
        <h2>{title}</h2>
        <div className="actions">
          <Pill tone="product-defect">疑似产品缺陷</Pill>
          <Pill tone={finding.severity}>{finding.severity}</Pill>
          <span className="muted small">第 {finding.round} 轮</span>
